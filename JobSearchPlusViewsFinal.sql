...
GO
CREATE VIEW [Number of job leads entered per day]
AS
SELECT Count(LeadID) as Leads, Day(RecordDate) as 'Day'
FROM Leads
GROUP BY Day(RecordDate)

Go
CREATE VIEW [Number of job leads entered per week]
AS
...
AS
SELECT JobTitle, CompanyName, concat(ContactFirstName,' ', ContactLastName) 'ContactName',
CT.Phone, CT.Email, Datediff(Day, ModifiedDate, getdate()) 'Days From Update'
FROM Leads L
Join Companies C
ON L.CompanyID = C.CompanyID
Join Contacts CT
ON L.ContactID = CT.ContactID
WHERE Datediff(Day, ModifiedDate, getdate()) >= 7
Go

CREATE VIEW [Number of leads by Source]
AS
SELECT  COUNT(LeadID) 'Leads',MAX(SourceName) 'Source'
FROM Leads L
Join Sources S
...
AS
SELECT CompanyName, CourtesyTitle, ContactFirstName, ContactLastName,
Title, CT.Phone, CT.Extension, CT.Fax, CT.Email, CT.Comments, C.Agency
FROM Contacts CT
Join Companies C
...
AS
SELECT  JobTitle,CompanyName, L.Descript, Location, Concat(ContactFirstName,' ',ContactLastName) 'PrimeContactName',
		CT.Phone, DateDiff(Day,ModifiedDate,Getdate()) 'Days Since Update'
FROM Leads L
Join Contacts CT
...
Go
CREATE VIEW [Search Log 30days]
AS
SELECT ActivityDate, ActivityType, JobTitle, CompanyName, 
Datediff(Day, ActivityDate, getdate()) 'Days from Last Activity', IIF(A.Complete = 1, 'YES','NO') 'Complete'
FROM Activities A
Join Leads L
...
Join Contacts CT
ON CT.ContactID = L.CompanyId
WHERE Datediff(Day, ActivityDate, getdate()) < 30 
GO
CREATE VIEW [Lead Report]
AS
SELECT L.[LeadID],L.[RecordDate],L.[JobTitle],L.[Descript],L.[EmploymentType],L.[Location],L.[Activity],
L.[CompanyId],L.[AgencyID],L.[ContactID],L.[SourceID],L.[Selected],L.[ModifiedDate],c.[CompanyName],
CONCAT(CT.[CourtesyTitle],',',CT.[ContactFirstName],'',CT.[ContactLastName]) AS CONTACT,CT.[Phone],
CT.[Extension],CT.[Email],S.[SourceName]
FROM Leads L
Join Companies C
ON L.CompanyID = C.CompanyID
Join Contacts CT
